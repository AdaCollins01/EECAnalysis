...
public:
  ...
  ~JDrawer(){
    ...
    if(fRightRowPad) delete fRightRowPad;
  }
  ...
  void DrawHistogram(TH1* histo, const char *drawOption = ""){
    DrawHistogram(histo,"","","",drawOption);
  }
  ...
  void CreateCanvasGraphRow(){
    ...
    double firstSeparator = 1 / (1 + relativePlotSizeLeftPad/relativePlotSizeMiddlePad + relativePlotSizeLeftPad/relativePlotSizeRightPad);
    ...
    fLeftRowPad->SetRightMargin(evenSmallerMargin); // Make the right margin small so that the pads fit nicely on top of each other
    ...
    fMiddleRowPad->SetLeftMargin(smallMargin); // Make the left margin small so that the pads fit nicely on top of each other
    fMiddleRowPad->SetRightMargin(smallMargin); // Make the right margin small so that the pads fit nicely on top of each other
    ...
    fRightRowPad->SetLeftMargin(smallMargin); // Make the left margin small so that the pads fit nicely on top of each other
   ...
   */
  void SetHistogramAppearance(double titoffx, double titoffy, double titsizex, double titsizey,
                              double labeloffx, double labeloffy, double labelsizex, double labelsizey,
                              int divx, int divy, int fontIndex){
