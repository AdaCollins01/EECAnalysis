...
void plotEEECxyClosure(){
  ...
  if(partonShower + dataSource != 1) { 
  	cout << "Please select either partonShower or dataSource." << endl;
	return;
  }

  TString dateFileString = "07212025";
 ...
 std::vector<std::pair<const char*, const char*>> closurePartonShowerLabels;
   ...
   for(auto RLBin : comparedRL){
	  ...
	  if(inputJustPythia == NULL || inputPythiaGeant == NULL || inputCMS == NULL){
	   ...
	   //		normalizationFactor += tempNumJets; // Adds all jets together
	   	...
	   	// justPythia
	   		...
	   		int yHighBinJustPythia = hEnergyEnergyEnergyCorrelatorJustPythia[iJetPtJustPythia][iTrackPtJustPythia]->GetYaxis()->FindBin(0.9999);
	   	...
	   	// pythiaGeant
	   		...
	   		int yHighBinPythiaGeant = hEnergyEnergyEnergyCorrelatorPythiaGeant[iJetPtPythiaGeant][iTrackPtPythiaGeant]->GetYaxis()->FindBin(0.9999);
	   	...
	   	// CMS
	   		...
	   		int yHighBinCMS = hEnergyEnergyEnergyCorrelatorCMS[iJetPtCMS][iTrackPtCMS]->GetYaxis()->FindBin(0.999);
	...
	if(invert){
	  ...
	  TString inputFileDire = "pythia/rootFiles/pythia_pp_eec_dire_6000events_07162025.root";
	...
	// Simple
		...
		int yHighBinSimple = histSimple->GetYaxis()->FindBin(0.9999);
	...
	// Dire
		...
		int yHighBinDire = histDire->GetYaxis()->FindBin(0.9999);
	...
	// Vincia
		...
		int yHighBinVincia = histVincia->GetYaxis()->FindBin(0.9999);
	...
	// Prepare a JDrawer for drawing purposes
	  ...
	  TString normalMethodString = "Area";
	  TString normalFileString = "normalArea";
	...
	TLegend *legend;
	if(partonShower) {legend = new TLegend(0.83,0.38,1.01,0.67);}
